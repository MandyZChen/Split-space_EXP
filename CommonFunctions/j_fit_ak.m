function [varargout] = j_fit_ak(x_data, y_data, fit_type, varargin)
%
...
if isa(fit_type, 'function_handle')
    ...
    switch lower(fit_type)
      ...
      case 'logistic1' % sigmoid 1
        start_point = [1 mean(x_data)];% [1 mean(x_data)];
      ...
      case {'s_curve','gauss_deriv'} % derivative of a gaussian
        ...
        case 'cum_gauss'
            scale = 0;shift = 0;
            start_point = [1*std(x_data) mean(x_data) ];
            f = make_cum_gauss(scale,shift);
...
%         plot(x_data,y_data,'MarkerEdgeColor',color,'MarkerFaceColor',color,'Marker','o','Line','none')
%         %axis([xmin xmax ymin ymax]);
%         set(gca,'FontSize',14);
%         hold on;
%         plot((min(x_data):(max(x_data)-min(x_data))/100:max(x_data)),y_predicted,'Color',[.3 .3 .3],'Line','-','LineWidth',2);
...
% 
%         plot(boot_params1,boot_params2,'Color',[.8 .4 .8],'Marker','o','Line','none'); hold on;
%         plot(a_boot_mean,b_boot_mean,'MarkerEdgeColor',[.05 .5 .8],'MarkerFaceColor',[.05 .5 .8],'Marker','o','Line','none');
...
varargout(length(fit_parameters)+1) = {boot_params};

varargout(length(fit_parameters)+2) = {b_boot_std};
...
function f_out = make_cum_gauss(scale,shift)
f_out = @(x,params) cum_gauss(x, scale, shift,params(1),params(2));
   
end

function retVal = cum_gauss(x, scale,shift,a, b)
 retVal = scale+shift+.5.*(1+erf((x-b)/(a*sqrt(2))));
end
